...
if ($arResult['CATALOG'] && isset($arResult['OFFERS']) && !empty($arResult['OFFERS']))
{
	...
	if (count($arResult["OFFERS"])>0){
		foreach($arResult["OFFERS"] as &$arItemOffer){
			...
			if ($arItemOffer["CATALOG_QUANTITY"]<=0 && ($arResult["CATALOG_CAN_BUY_ZERO"] == "N" || $arResult["CATALOG_NEGATIVE_AMOUNT_TRACE"] == "N")){
	...
	foreach ($arResult['OFFERS'] as $keyOffer => $arOffer)
	{
		...
		if (empty($arResult['MIN_PRICE']) && $arOffer['CAN_BUY'])
		{
                    ...
                    $minPrice = (isset($arOffer['RATIO_PRICE'])) ? $arOffer['RATIO_PRICE'] : $arOffer['MIN_PRICE'];
                    $arResult['MIN_PRICE'] = ($minPrice &&  $arOffer['RATIO_PRICE']['VALUE'] < $arResult['MIN_PRICE']['VALUE']) ? $minPrice : $arResult['MIN_PRICE'];
                }
		$arSKUProps = false;
...
if ($arParams["OFFERS_VIEW"] == "CHOISE") {
    ...
    //формирование списка св-в предложений
    ...
    //получение значений свойств типа список. сбор id свойств типа справочник и привязка к элементу
    ...
    //получение значений свойств типа привязка к элементу
    ...
    //получение значений свойств типа справочник
