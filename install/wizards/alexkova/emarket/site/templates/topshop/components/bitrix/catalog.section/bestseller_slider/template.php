...
if (!\Bitrix\Main\Loader::includeModule("alexkova.emarket")) return;
...
<div class="sale-carousel lslider xs-hide" id="c<?=intval($_REQUEST["ID"])?>">
	...
	if (is_array($arItem["PREVIEW_PICTURE"])){
		$img = \Alexkova\Emarket\Catalog\Image::getResizeImage($arItem["PREVIEW_PICTURE"]["ID"], 150);
	}elseif(is_array($arItem["DETAIL_PICTURE"])){
		$img = \Alexkova\Emarket\Catalog\Image::getResizeImage($arItem["DETAIL_PICTURE"]["ID"], 150);
	}
	...
	?>
		...
		?>
			...
			<li class="span_1_of_4 sm-span_1_of_3 xs-span_2_of_4 sale-cart" id="<?=$strMainID?>">
					...
					<div class="sale-cart-price-line">
                            <?if ($arParams["USE_PRICE_COUNT"] == 'Y' && count($arItem['OFFERS']) == 0) {?>
                                ...
                                if (count($arItem["PRICE_MATRIX"]["ROWS"]) == 1) {
                                    $currency = ($firstPrice[0]["CURRENCY"] == "RUB") ? "руб" : $firstPrice[0]["CURRENCY"];?>
                                ...
                                <?} else {
                                    foreach ($arItem["PRICE_MATRIX"]["ROWS"] as $cell => $quantity) {
                                        $currency = ($firstPrice[$cell]["CURRENCY"] == "RUB") ? "руб" : $firstPrice[$cell]["CURRENCY"];
                                        ...
                                        <div class="sale-cart-quantity-price <?=$colClass?>">
                                            <?if ($quantity["QUANTITY_TO"] == 0) {?>
                                                <?="от ".$quantity["QUANTITY_FROM"]." шт"?>
                                            <?} else {?>
                                                <?=$quantity["QUANTITY_FROM"]." - ".$quantity["QUANTITY_TO"]." шт"?>
                                            <?}?>
                                        </div>
						...
						<div class="emarket-sale-buttons">
							...
							<?if (count($arItem["OFFERS"])<=0  && $arItem["CAN_BUY"]){?>
								<form class="basket_action" id="cart_form_<?=$arItem["ID"]?>">
									...
									global $eMarketBasketData;
									$addClass="";
									if (is_array($eMarketBasketData["DELAY"]) && in_array($arItem["ID"], $eMarketBasketData["DELAY"])) $addClass=" active";
									?>
									<?
									...
									if (is_array($eMarketBasketData["DELAY"]) && in_array($arItem["ID"], $eMarketBasketData["ITEMS"])){
										$compareMSG = " active";
									...
									$addClass = '';
									if (is_array($eMarketBasketData["DELAY"]) && in_array($arItem["ID"],$eMarketBasketData["ITEMS"])) $addClass=' active';
									?>
	...
	<input type="button" class="slide-button-prev sm-hide xs-hide" id="c<?=intval($_REQUEST["ID"])?>_prev" data-fix="c<?=intval($_REQUEST["ID"])?>" value=" ">
	<input type="button" class="slide-button-next sm-hide xs-hide" id="c<?=intval($_REQUEST["ID"])?>_next" data-fix="c<?=intval($_REQUEST["ID"])?>" value=" ">
	<div class="clear"></div>
