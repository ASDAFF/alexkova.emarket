...
<div id="basket_items_list">
					...
					<tr id="<?=$arItem["ID"]?>">
							...
							?>
								...
								<td class="item">
									<h2 class="bx_ordercart_itemtitle">
                                                                            <?$url = (isset($arItem["PARENT"]["DETAIL_PAGE_URL"])) ? $arItem["PARENT"]["DETAIL_PAGE_URL"] : $arItem["DETAIL_PAGE_URL"];?>
										<?if (strlen($url) > 0):?><a href="<?=$url?>"><?endif;?>
											<?=$arItem["NAME"]?>
										<?if (strlen($url) > 0):?></a><?endif;?>
									</h2>
												...
												<td>
													...
													<div class="basket_quantity_control">
														<?if (floatval($arItem["MEASURE_RATIO"]) != 0):?>
															<input type="button" value="-" class="minus quantity-button-minus" onclick="setQuantity(<?=$arItem["ID"]?>, <?=$arItem["MEASURE_RATIO"]?>, 'down', <?=$useFloatQuantityJS?>);"/>
														<?endif;?>
														...
														>
													<?if (floatval($arItem["MEASURE_RATIO"]) != 0):?>
														<input type="button" value="+" class="plus quantity-button-plus" onclick="setQuantity(<?=$arItem["ID"]?>, <?=$arItem["MEASURE_RATIO"]?>, 'up', <?=$useFloatQuantityJS?>);"/>
													<?endif;?>
								...
								<td class="price">
										<div class="current_price" id="current_price_<?=$arItem["ID"]?>">
                                                                                    <?=$arItem["PRICE_FORMATED"]?><?
                                                                                            if(isset($arItem["MEASURE_TEXT"]) && isset($arParams['SHOW_UNIT_MEASUREMENT']) && $arParams['SHOW_UNIT_MEASUREMENT'] == "Y")
						...
						?>
							<td class="control">
								...
								if ($bDeleteColumn):
								?>
									<a class="icon-button-delete" href="<?=str_replace("#ID#", $arItem["ID"], $arUrls["delete"])?>"><?=GetMessage("SALE_DELETE")?></a>
								<?
								endif;
								if ($bDelayColumn):
								?>
									<a class="icon-button-delay no-shadow" href="<?=str_replace("#ID#", $arItem["ID"], $arUrls["delay"])?>"><?=GetMessage("SALE_DELAY")?></a>
								<?
								endif;
								?>
							</td>
	...
	<div class="bx_ordercart_order_pay">
			...
			<table class="bx_ordercart_order_sum">
				...
				<?endif;?>
					...
					<tr>
						...
						<td class="fwb custom_t2" id="allSum_FORMATED">
		...
		<div class="bx_ordercart_order_pay_center">
			...
			<a class="color-button" href="javascript:void(0)" onclick="checkOut();" class="checkout"><?=GetMessage("SALE_ORDER")?></a>
		</div>
	</div>
</div>
<?
