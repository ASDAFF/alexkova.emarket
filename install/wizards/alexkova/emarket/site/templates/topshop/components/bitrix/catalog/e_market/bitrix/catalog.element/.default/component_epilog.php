					...
					<?foreach($arResult["OFFERS"] as $offer):
						...
						<tr>
							...
							<td class="emarket-offers-price">
								...
								<div class="emarket-offer-price emarket-format-price" id="price_<? echo $offer['ID']; ?>"><? echo $offer['MIN_PRICE']['PRINT_DISCOUNT_VALUE']; ?></div>
							...
							<td class="emarket-offers-sale xs-hide">
								<form class="basket_action">
									<?if ('Y' == $arParams['USE_PRODUCT_QUANTITY']):?>
										...
										<input type="text" value="1" class="quantity-text sm-hide xs-hide" name="quantity" id="quantity-<?=$offer["ID"]?>">
										<input type="hidden" value="<?=$offer["ID"]?>" name="item">
										<input type="hidden" value="ADDTOCART" name="action">
			...
			<div class="emarket-detail-area-container"  id="emarket-comments-tab">
				...
				<?$APPLICATION->IncludeComponent("bitrix:catalog.comments",
					"emarket_comments", Array(
						...
						"WIDTH" => "",	// Ширина
						"COMMENTS_COUNT" => "10",	// Количество показываемых комментариев
						"BLOG_USE" => $arParams["BLOG_USE"],	// Использовать комментарии
						"FB_USE" => $arParams["FB_USE"],	// Использовать Facebook
						"FB_APP_ID" => $arParams["FB_APP_ID"],
						"VK_USE" => $arParams["VK_USE"],	// Использовать Вконтакте
						"VK_API_ID" => $arParams["VK_API_ID"],
						"CACHE_TYPE" => $arParams["CACHE_TYPE"],	// Тип кеширования
						"CACHE_TIME" => $arParams["CACHE_TIME"],	// Время кеширования (сек.)
						"BLOG_TITLE" => "",
						"BLOG_URL" => "catalog_comments"."_".SITE_ID,//$arParams["BLOG_URL"],
						"PATH_TO_SMILE" => "",
...
if (isset($arResult["ACCESSORIES"]) && count($arResult["ACCESSORIES"])>0){
